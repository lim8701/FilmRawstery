...
def gpu_available() -> bool:
    """GPU 가속 EP 존재 여부 — 모델 다운로드/세션 생성 없이 확인(토글 시 CPU 확인용).
    설치된 EP 기준이라 드물게 세션 생성 시 장치 초기화가 실패할 수 있음(그 경우 CPU 폴백)."""
    import onnxruntime as ort
    avail = set(ort.get_available_providers())
    return any(p in avail for p in _ACCEL_PROVIDERS)
...
def ensure_model(progress=None) -> str:
    """모델 파일 보장(없으면 다운로드, ~72MB). progress(0..1) 콜백 옵션. 경로 반환."""
...
def denoise_rgb(rgb: np.ndarray, progress=None, cancel=None,
                drift_sigma: float = DRIFT_SIGMA, pace: float = 0.0) -> np.ndarray:
